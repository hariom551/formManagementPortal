...
function PollingStationAllotment() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (content) {
          const PollingStationAllotment = data.find(item =>{  return item.Id == content});
  ...
  const handleEdit = async (e) => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        window.location.href = '/PollingStationAllotment';

        console.log("PollingStationAllotment Updated successfully.");
      } else {
        console.error("Error in Updating PollingStationAllotment:", result.statusText);
      }
    } catch (error) {
  ...
  const handleDelete = async (Id) =>{
    ...
    try {
      ...
      if (result.ok) {
        window.location.reload();
        console.log("PollingStationAllotment Added Successfully successfully.");
      } else {
        console.error("Error in Adding PollingStationAllotment:", result.statusText);
      }
    } catch (error) {
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <>
            ...
            <Link
              to={{ pathname: "/PollingStationAllotment", search: `?content=${row.original.Id}` }}
            >
              Edit
            </Link>
          </Button>
  ...
  return (
    <main className="bg-gray-100">
      <div className="container py-4 pl-6 text-black">
        ...
        <Form onSubmit={content ? handleEdit : handleSubmit} className="PollingStationAllotment-form">
          <Row className="mb-3">
