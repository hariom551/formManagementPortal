...
function OutgoingForms() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/FormsAdmin/outFormDetails`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                    }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const result = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/FormsAdmin/addOutForm`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
